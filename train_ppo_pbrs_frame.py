...
from value_based_deep_helper_functions import FrameStackWrapper
...
class PPO:
    def __init__(self, env, args, activation=F.relu):
        ...
        self.actionDim = 5
        self.stateDim = 18 * args.num_frames
        ...
        if args.load_weights is not None:
            ...
            # Optional: Load Value network if it exists in a separate file,
            # otherwise it trains from scratch to learn the new wall penalties.
        # ---------------------------
        ...
        self.buffer = RolloutBuffer(args.steps_per_epoch, self.stateDim)
    ...
    def trainAgent(self):
        ...
        for epoch in pbar:
            ...
            for t in range(self.args.steps_per_epoch):
                ...
                self.buffer.store(s, a, r, v, logp, done)
                s = s_next
            ...
            with torch.no_grad():
                if ep_step == 0:
                    ...
                    # Otherwise, we bootstrap from the true s_next
    ...
    def trainNetwork(self, advs_np, returns_np):
        ...
        b_values = torch.tensor(self.buffer.values, dtype=torch.float32).to(
            self.device
        )  #
        ...
        for _ in range(self.args.opt_epochs):
            ...
            for start in range(0, dataset_size, self.args.batchSize):
                ...
                # Calculate both losses and take the maximum (pessimistic bound)
...
def main():
    ...
    ap.add_argument("--out", type=str, default="weights_ppo_pbrs_frame.pth")
    ...
    # PPO Specific Hyperparameters (Optimized for Sparse Navigation)
    ...
    # This will now run even if you Ctrl+C !
