...
class D3QN_PER:
    def __init__(
        ...
        **kwargs,
        ...
        # --- CURRICULUM LEARNING WEIGHT LOADING ---
        load_path = kwargs.get("load_weights", None)
        if load_path is not None:
            print(f"Loading weights from {load_path} for Phase 2...")
            sd = torch.load(load_path, map_location=self.device)
            if isinstance(sd, dict) and "state_dict" in sd:
                sd = sd["state_dict"]
            self.nnOnline.load_state_dict(sd, strict=True)
        elif kwargs.get("std_init", True):
            self.nnOnline.apply(init_weights)
        # ------------------------------------------
...
def main():
    ...
    ap.add_argument("--out", type=str, default="weights_D3qn_PER_pbrs_frame.pth")
    ...
    ap.add_argument("--decay_rate", type=float, default=(0.7 / 1.0) ** (1 / 2499999))
