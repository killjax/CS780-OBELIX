...
def _load_once():
    ...
    wpath = os.path.join(submission_dir, "weights_D3qn_PER_frame_stack.pth")
...
def policy(obs: np.ndarray, rng: np.random.Generator) -> str:
    ...
    if len(_FRAME_QUEUE) == 0:
        # If first step, fill queue with the starting frame
        for _ in range(4):
            _FRAME_QUEUE.append(obs)
    else:
        # Push newest frame, oldest drops off naturally
        _FRAME_QUEUE.append(obs)

    # Flatten the queue into 72 values
    stacked_obs = np.concatenate(list(_FRAME_QUEUE), axis=0)

    # Pass the 72-value array to the model
