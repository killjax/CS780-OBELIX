...
def q_learning_control(environment, config=None):
    ...
    for e in tqdm.tqdm(range(max_episodes), desc="Training Episodes"):
        ...
        while not done:
            ...
            td_target = r
            if not done:
                td_target += discount_factor * (np.max(Q[n_s]))
...
def main():
    ...
    ap.add_argument("--out", type=str, default="q_table1.npy")
    ...
    Q_q_learning = q_learning_control(env, config=CONFIG)

    with open(args.out, "wb") as f:
        np.save(f, Q_q_learning)
